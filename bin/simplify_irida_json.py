    ...
    def flatten(item, name=""):
        if type(item) is dict:
            ...
            for i in range(len(item)):
                flatten(
                    item[i], str(name) + str(i + 1) + "."
                )  # i + 1 because biologists

        else:
            result[str(name)[:-1]] = (
                item  # [:-1] avoids the "." appended on the previous recursion
            )
...
def main():
    ...
    for sample in input_json["metadata"]["samples"]:
        input_json["metadata"]["samples"][sample] = flatten_dictionary(
            input_json["metadata"]["samples"][sample]
        )
